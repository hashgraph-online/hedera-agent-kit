...
import { HederaAgentKit, PluginConfig } from './';
...
export class HederaConversationalAgent {
  ...
  public async initialize(): Promise<void> {
    ...
    if (toolsFromKit.length === 0) {
      this.logger.warn(
        'No tools were loaded into HederaAgentKit. The conversational agent may not function correctly.'
      );
    }
    ...
    try {
      ...
      if (result.intermediateSteps && result.intermediateSteps.length > 0) {
        ...
        if (lastStep && lastStep.observation) {
          ...
          try {
            const toolJsonOutput = JSON.parse(
              lastStep.observation as string
            ) as ParsedToolOutput;
            ...
            this.logger.debug(
              'Tool observation was not JSON or not structured as expected, using as raw output.',
              lastStep.observation
            );
            response.output =
              typeof lastStep.observation === 'string'
                ? lastStep.observation
                : JSON.stringify(lastStep.observation);
          }
        }
      ...
      if (
        ...
        if (response.transactionBytes) {
          response.output = `Transaction bytes prepared (for payer ${this.hederaKit.signer
            .getAccountId()
            .toString()}): ${response.transactionBytes}`;
        } else if (response.receipt) {
          response.output = `Transaction executed by agent. Receipt: ${JSON.stringify(
            response.receipt
          )}`;
          ...
          response.output =
            'Agent did not provide specific structured output from tool.';
        }
      }
      return response;
    } catch (error: unknown) {
