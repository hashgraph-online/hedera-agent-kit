...
import { HederaCreateTopicTool, HederaDeleteTopicTool } from '../../src/langchain/tools/hcs'; // Assuming path to HCS tools index or specific file
...
function getToolOutputFromResult(agentResult: any): any {
  ...
  if (agentResult.intermediateSteps && agentResult.intermediateSteps.length > 0) {
    ...
    if (typeof observation === 'string') {
      try { toolOutputData = JSON.parse(observation); }
      catch (e: any) { throw new Error(`Failed to parse observation string from intermediateStep. String was: "${observation}". Error: ${e.message}`); }
    } else if (typeof observation === 'object' && observation !== null) {
      toolOutputData = observation; console.log("Observation from intermediateStep was already an object, using directly:", toolOutputData);
    } else { console.warn('Observation in last intermediate step was not a string or a recognized object. Full step:', lastStep); }
  }
  if (!toolOutputData) {
    console.warn('Could not find usable tool output in intermediateSteps. Attempting to parse agentResult.output.', `agentResult.output: ${agentResult.output}`);
    if (!(agentResult.intermediateSteps && agentResult.intermediateSteps.length > 0)) {
      if (typeof agentResult.output === 'string') {
        try { toolOutputData = JSON.parse(agentResult.output); console.warn('Parsed agentResult.output as a fallback.'); }
        catch (e: any) { throw new Error(`No intermediate steps, and agentResult.output was not valid JSON. Output: "${agentResult.output}". Error: ${e.message}`); }
      } else { throw new Error("No intermediate steps, and agentResult.output is not a string."); }
    } else { throw new Error("Intermediate steps found, but observation was not a usable string or object. See logs for details."); }
  }
...
describe('Hedera HCS Tools Integration Tests', () => {
  ...
  afterAll(async () => {
    if (kit && createdTopicIds.length > 0) {
      console.log(`Attempting to clean up ${createdTopicIds.length} created topic(s)...`);
      const deleteTool = new HederaDeleteTopicTool({ hederaKit: kit });
      const agentExecutor = await createTestAgentExecutor(deleteTool, openAIApiKey);
      for (const topicId of createdTopicIds) {
        try {
          ...
          expect(result.success, `Cleanup Failed for ${topicId.toString()}: ${result.error}`).toBe(true);
          if (result.receipt) {
            expect(result.receipt.status).toEqual('SUCCESS');
            console.log(`Successfully cleaned up topic ${topicId.toString()}`);
          }
        } catch (error) {
          console.error(`Failed to clean up topic ${topicId.toString()}:`, error);
        }
      }
    }
  });
  ...
  describe('HederaCreateTopicTool', () => {
    it('should create a new HCS topic with a memo and admin key', async () => {
      ...
      expect(result.success, `CreateTopic Test Failed: ${result.error}`).toBe(true);
      expect(result.receipt).toBeDefined();
      expect(result.receipt.status).toEqual('SUCCESS');
      expect(result.receipt.topicId?.toString()).toMatch(new RegExp('^0\\.0\\.\\d+$'));

      if (result.receipt.topicId) {
        createdTopicIds.push(result.receipt.topicId);
        console.log(`Created HCS Topic ${result.receipt.topicId.toString()} with memo '${topicMemo}'.`);
      }
    });
  });
  ...
  describe('HederaSubmitMessageToTopicTool', () => {
    ...
    beforeAll(async () => {
      ...
      const agentExecutorCreate = await createTestAgentExecutor(createTopicTool, openAIApiKey);
      ...
      const agentResultCreate = await agentExecutorCreate.invoke({ input: createPrompt });
      const resultCreate = getToolOutputFromResult(agentResultCreate);
      expect(resultCreate.success, `Setup for SubmitMessage (Topic Creation) failed: ${resultCreate.error}`).toBe(true);
      expect(resultCreate.receipt?.topicId, 'Topic creation for message submission failed to return topicId').toBeDefined();
      topicIdForMessages = resultCreate.receipt!.topicId!;
      createdTopicIds.push(topicIdForMessages); // Ensure cleanup
      console.log(`Created topic ${topicIdForMessages.toString()} for message submission tests.`);
    });

    it('should submit a message to the topic', async () => {
      const tool = new HederaSubmitMessageTool({ hederaKit: kit });
      const agentExecutor = await createTestAgentExecutor(tool, openAIApiKey);
      const messageContent = `Test message from HederaAgentKit: ${generateUniqueName('msg')}`;
      const prompt = `Submit the following message to topic ${topicIdForMessages.toString()}: "${messageContent}"`;

      const agentResult = await agentExecutor.invoke({ input: prompt });
      const result = getToolOutputFromResult(agentResult);

      expect(result.success, `SubmitMessage Test Failed: ${result.error}`).toBe(true);
      expect(result.receipt).toBeDefined();
      expect(result.receipt.status).toEqual('SUCCESS');
      ...
      console.log(`Successfully submitted message to topic ${topicIdForMessages.toString()}. Sequence number: ${result.receipt.topicSequenceNumber}`);
    });
  });
  ...
  describe('HederaUpdateTopicTool', () => {
    ...
    beforeAll(async () => {
      const createTopicTool = new HederaCreateTopicTool({ hederaKit: kit });
      const agentExecutorCreate = await createTestAgentExecutor(createTopicTool, openAIApiKey);
      const operatorPublicKeyDer = (await kit.signer.getPublicKey()).toStringDer(); // Re-introduce
      ...
      const agentResultCreate = await agentExecutorCreate.invoke({ input: createPrompt });
      const resultCreate = getToolOutputFromResult(agentResultCreate);
      expect(resultCreate.success, `Setup for UpdateTopic (Topic Creation) failed: ${resultCreate.error}`).toBe(true);
      expect(resultCreate.receipt?.topicId, 'Topic creation for update test failed to return topicId').toBeDefined();
      topicToUpdateId = resultCreate.receipt!.topicId!;
      createdTopicIds.push(topicToUpdateId); 
      console.log(`Created topic ${topicToUpdateId.toString()} with memo '${initialMemo}' for update tests.`);
    });
    ...
    it('should update the topic memo', async () => {
      ...
      expect(result.success, `UpdateTopic Memo Test Failed: ${result.error}`).toBe(true);
      expect(result.receipt).toBeDefined();
      expect(result.receipt.status).toEqual('SUCCESS');
      console.log(`Successfully updated memo for topic ${topicToUpdateId.toString()} to '${newMemo}'.`);
  ...
  describe('HederaDeleteTopicTool (Dedicated Test)', () => {
    ...
    beforeEach(async () => {
      ...
      const agentResultCreate = await agentExecutorCreate.invoke({ input: createPrompt });
      const resultCreate = getToolOutputFromResult(agentResultCreate);
      expect(resultCreate.success, `Setup for DeleteTopic Test (Topic Creation) failed: ${resultCreate.error}`).toBe(true);
      expect(resultCreate.receipt?.topicId, 'Deletable topic creation failed to return topicId').toBeDefined();
      topicToDeleteId = resultCreate.receipt!.topicId!;
      // Not adding to global createdTopicIds as this test will delete it.
      console.log(`Created deletable topic ${topicToDeleteId.toString()} for dedicated delete test.`);
    });
    ...
    it('should delete the topic successfully', async () => {
      ...
      expect(result.success, `DeleteTopic Test Failed: ${result.error}`).toBe(true);
      expect(result.receipt).toBeDefined();
      expect(result.receipt.status).toEqual('SUCCESS');
      console.log(`Successfully deleted topic ${topicToDeleteId.toString()} in dedicated test.`);
    });
  });
}); 